...
def main():
 ...
 for n in NS:
  for retries,w0,wmax in SETTINGS:
   ...
   for i in range(RUNS): rows.append(run_metrics(run_dir=raw/f'csma_ca_{i:04d}',run_index=i,device_count=n,cycle_time=T,duration=D,carrier_sense_duration_ms=5,airtime_ms=req.airtime_ms,coupling_parameter=0,window_mode='fixed_anchor',algorithm='CSMA_CA',extra={'csma_w0_ms':w0,'csma_w_max_ms':wmax,'csma_max_retries':retries}))
 ...
 cond=df.groupby(keys,as_index=False).agg(run_count=('run_id','count'),overall_per_percent_median=('overall_per_percent','median'),backoff_retry_total=('backoff_retry_total','sum'),retry_exhausted_abandon_total=('retry_exhausted_abandon_total','sum'),ttu_reach_rate_percent=('ttu_cycle',lambda x:100*x.notna().mean()),max_convergence_rate_percent=('max_converged',lambda x:100*x.mean()),mingap_convergence_rate_percent=('mingap_converged',lambda x:100*x.mean()),final_10_cycle_new_max_abs_dev_median=('final_10_cycle_new_max_abs_dev','median'),final_10_cycle_min_gap_median=('final_10_cycle_min_gap_median','median'))
 ...
 (OUT/'metadata.json').write_text(json.dumps({'schema_version':3,'algorithm':'CSMA_CA','condition_count':conditions,'runs_per_condition':RUNS,'anchor_policy':'Fixed anchor: no backoff carry-over; next send is s_i+(k+1)T.','intended_time_policy':'Abandonment intended time is the original scheduled anchor.'},indent=2)+'\n',encoding='utf-8')
