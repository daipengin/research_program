...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run and aggregate the N-sweep v2 experiment.")
    ...
    if args.smoke:
        ...
        raw_root = PROJECT_ROOT / "outputs" / "n_sweep_v2_smoke_180cycles" / "raw"
        results_root = PROJECT_ROOT / "results" / "n_sweep_v2_smoke_180cycles"
    else:
        functions = FUNCTIONS
    ...
    validate_master_prefixes(MASTER_PATH)
    log(
        log_path,
        "validation master_unique_prefixes=PASS "
        f"N={list(DEVICE_COUNTS)} runs={RUNS_PER_CONDITION}",
    )
        ...
        for device_count in device_counts:
            ...
            for k_value in k_values:
                ...
                request = _simulation_request_for_k(
                    graph_id=f"n_sweep_v2_{function_name}_{device_count}_{k_value}",
                ...
                log(
                    ...
                    f"runs={runs_per_condition} "
                    f"elapsed_sec={time.perf_counter() - condition_started:.3f}",
                )
    ...
    metadata = {
        "schema_version": 2,
        "experiment": "n_sweep_v2_smoke" if args.smoke else "n_sweep_v2",
        ...
        "carrier_sense_duration_ms": CARRIER_SENSE_DURATION_MS,
        "airtime_ms": airtime_ms,
        "occupied_time_ms": occupied_time_ms,
        "minimum_collision_free_gap_rad": 2.0 * math.pi * occupied_time_ms / NOMINAL_CYCLE_TIME_MS,
        "epsilon_tolerance_rad_by_device_count": epsilon_by_n,
        "consecutive_convergence_cycles": CONSECUTIVE_CYCLES,
        "formal_convergence_definition": (
            "First cycle of 10 consecutive cycles with new_max_abs_dev < epsilon_tol. "
            "Non-converged runs are right-censored: run cycle is NULL and converged=false; "
            "condition median treats them as +infinity and is NULL when convergence rate <=50%."
        ),
        "auxiliary_mean_definition": "Same rule using new_mean_abs_dev; auxiliary only.",
        "mingap_convergence_definition": (
            "First cycle of 10 consecutive cycles with min_gap_rad > "
            "2*pi*occupied_time_ms/cycle_time_ms; same censoring rule."
        ),
        "ttu_window_cycles": 10,
        "ttu_per_threshold_percent": 1.0,
        "master_seed": MASTER_SEED,
        "initial_phase_sampling": "uniform_without_replacement",
        ...
        "changes_from_v1": [
            "unique-within-run initial phase master with a new seed",
            "formal max-gap-deviation convergence with censored condition median",
            "min_gap_rad metric and collision-free min-gap convergence",
            "20-point common K grid",
        ],
    ...
    log(log_path, f"finished conditions={condition_count} runs={run_count} elapsed_sec={elapsed_total:.3f}")
    return 0


def experiment_params(
    *, function_name: str, device_count: int, k_values: tuple[int, ...],
    runs_per_k: int, max_workers: int,
) -> dict[str, Any]:
    return {
...
def initial_starts(device_count: int, run_count: int) -> tuple[tuple[int, ...], ...]:
    return _initial_start_times_by_run(
        experiment_params(
            function_name="KURAMOTO", device_count=device_count, k_values=(1,),
            runs_per_k=run_count, max_workers=1,
        )
    )


def validate_master_prefixes(path: Path) -> None:
    data = json.loads(path.read_text(encoding="utf-8"))
    if data.get("seed") != MASTER_SEED:
        raise ValueError(f"unexpected master seed in {path}")
    if data.get("sampling") != "uniform_without_replacement":
        raise ValueError(f"unexpected sampling mode in {path}")
    runs = data.get("start_times_by_run")
    if not isinstance(runs, list) or len(runs) != RUNS_PER_CONDITION:
        raise ValueError("master must contain exactly 50 runs")
    for n in DEVICE_COUNTS:
        for run_index, values in enumerate(runs):
            prefix = values[:n]
            if len(prefix) != n or len(set(prefix)) != n:
                raise ValueError(f"duplicate or short prefix: N={n}, run={run_index}")


def configured_airtime_ms() -> float:
    return calculate_lora_airtime_ms(
        LoRaAirtimeConfig(
            payload_bytes=37, spreading_factor=7, bandwidth_hz=500_000,
            coding_rate_denominator=5, preamble_symbols=8, explicit_header=True,
            crc_enabled=True, low_data_rate_optimize=None,
        )
    )
...
def aggregate_condition_runs(
    *, db_path: Path, function_name: str, device_count: int, k_value: int,
    ...
    with closing(sqlite3.connect(db_path)) as conn:
        ...
        runs = conn.execute("SELECT * FROM runs ORDER BY random_run_index, run_id").fetchall()
        for run in runs:
            run_id = str(run["run_id"])
            send_df = add_detection_time_column(pd.read_sql_query(
                """SELECT time, oscillator_id, send_count, transmission_end_time,
                          transmission_time_ms FROM send_log
                   WHERE run_id = ? ORDER BY time, oscillator_id""",
                conn, params=(run_id,),
            ))
            cycle_df = pd.read_sql_query(
                "SELECT cycle_index, cycle_start_time FROM calculated_cycle_data WHERE run_id = ? ORDER BY cycle_index",
                conn, params=(run_id,),
            )
            ...
            new_phase_df = compute_mean_abs_gap_error_per_cycle(
                send_df=send_df, cycle_starts=cycle_starts, num_devices=device_count,
            ...
            phase_df = legacy_phase_df.merge(
                new_phase_df[["cycle_index", *new_columns]], on="cycle_index",
                how="outer", sort=True,
            )
            ...
            epsilon = tolerance_rad(
                device_count=device_count, nominal_cycle_time_ms=float(run["cycle_time"]),
                carrier_sense_duration_ms=carrier_sense_ms, airtime_ms=airtime_ms,
            )
            min_gap_threshold = 2.0 * math.pi * occupied_time_ms / float(run["cycle_time"])
            phase_cycles = phase_df["cycle_index"].to_numpy(dtype=np.int64)
            aux_mean_convergence = first_consecutive_below(
            ...
            mingap_convergence = first_consecutive_above(
                cycle_indices=phase_cycles,
                values=phase_df["min_gap_rad"].to_numpy(dtype=np.float64),
            ...
            rows.append({
                ...
                "mingap_convergence_cycle": mingap_convergence,
                "mingap_converged": mingap_convergence is not None,
                "aux_mean_convergence_cycle": aux_mean_convergence,
                "aux_mean_converged": aux_mean_convergence is not None,
                "final_10_cycle_new_mean_abs_dev": float(final_phase["new_mean_abs_dev"].mean()),
                "final_10_cycle_new_max_abs_dev": float(final_phase["new_max_abs_dev"].mean()),
                "final_10_cycle_min_gap_median": float(final_phase["min_gap_rad"].median()),
...
def expected_cycles_from_run(run: sqlite3.Row) -> int:
    durations = []
    for item in str(run["ranges"] or "").split("|"):
        parts = item.split(":")
        if len(parts) == 3:
...
def aggregate_conditions(run_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for keys, group in run_df.groupby(["coupling_function", "device_count", "k"], sort=True):
        ...
        rows.append({
            ...
            "max_convergence_cycle_censored_median": censored_convergence_median(
                group["max_convergence_cycle"], group["max_converged"]
            ),
            "max_convergence_rate_percent": float(group["max_converged"].mean() * 100.0),
            "mingap_convergence_cycle_censored_median": censored_convergence_median(
                group["mingap_convergence_cycle"], group["mingap_converged"]
            ),
            "mingap_convergence_rate_percent": float(group["mingap_converged"].mean() * 100.0),
            "aux_mean_convergence_cycle_censored_median": censored_convergence_median(
                group["aux_mean_convergence_cycle"], group["aux_mean_converged"]
            ),
            "aux_mean_convergence_rate_percent": float(group["aux_mean_converged"].mean() * 100.0),
            "final_10_cycle_new_mean_abs_dev_median": float(group["final_10_cycle_new_mean_abs_dev"].median()),
            "final_10_cycle_new_max_abs_dev_median": float(group["final_10_cycle_new_max_abs_dev"].median()),
            "final_10_cycle_min_gap_median": float(group["final_10_cycle_min_gap_median"].median()),
            "simultaneous_collision_count_total": int(group["simultaneous_collision_count"].sum()),
        })
